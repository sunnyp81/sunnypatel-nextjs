// Historical collection method behind the July 2026 CTR edition.
// Intentional legacy runs require --collect-legacy and CTR_STUDY_OUT inside tmp.
// Public historical downloads are regenerated by export-ctr-study.mjs.
import { createSign } from "node:crypto";
import { readFileSync, writeFileSync } from "node:fs";
import { fileURLToPath } from "node:url";
import { resolve, relative, isAbsolute } from "node:path";

// Legacy method retained for inspection. Never overwrite the historical public
// edition from a fresh unfiltered extraction. New UK research needs a fixed panel.
if (!process.argv.includes("--collect-legacy") || !process.env.CTR_STUDY_OUT) {
  throw new Error("Historical collector disabled by default. To inspect the legacy method, use --collect-legacy and CTR_STUDY_OUT inside this repository's ignored tmp directory. Use export-ctr-study.mjs to regenerate public historical downloads.");
}
const scratch = fileURLToPath(new URL("../tmp/", import.meta.url));
const OUT = resolve(process.env.CTR_STUDY_OUT);
const relativeOutput = relative(scratch, OUT);
if (!relativeOutput || relativeOutput.startsWith("..") || isAbsolute(relativeOutput)) {
  throw new Error("CTR_STUDY_OUT must be a file inside this repository's ignored tmp directory.");
}
