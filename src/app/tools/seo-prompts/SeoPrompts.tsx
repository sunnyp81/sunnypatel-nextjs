...
const button = 'min-h-11 rounded-md border border-white/20 px-3 py-2 text-sm font-semibold transition-colors hover:bg-white/10 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brand disabled:cursor-not-allowed disabled:opacity-50';
const fieldClass = 'mt-2 w-full rounded-md border border-white/25 bg-background px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand';
...
function PromptCard({ prompt, values, setValues }: { prompt: Prompt; values: Record<string, string>; setValues: (values: Record<string, string>) => void }) {
  ...
  return (
    <article id={prompt.id} className="min-w-0 scroll-mt-28 rounded-lg border border-white/15 bg-white/[0.02] p-5 sm:p-6">
      <p className="text-xs font-semibold tracking-wide text-brand">{prompt.category}</p>
      ...
      <details className="mt-5 border-t border-white/10 pt-4">
        ...
        <pre tabIndex={0} aria-label={`${prompt.title} template`} className="mt-3 max-h-80 overflow-auto whitespace-pre-wrap break-words rounded-md bg-black/20 p-4 font-mono text-xs leading-relaxed text-foreground focus-visible:outline-2 focus-visible:outline-brand">{preparePrompt(prompt)}</pre>
        <button type="button" onClick={() => copy(false)} className={`${button} mt-3`}>Copy template</button>
      </details>
      <details className="mt-3 border-t border-white/10 pt-3" onToggle={event => {
        ...
        <pre tabIndex={0} aria-label={`${prompt.title} personalised preview`} className="mt-3 max-h-80 overflow-auto whitespace-pre-wrap break-words rounded-md bg-black/20 p-4 font-mono text-xs leading-relaxed text-foreground focus-visible:outline-2 focus-visible:outline-brand">{prepared}</pre>
      ...
      <p className="mt-4 border-t border-white/10 pt-4 text-xs leading-relaxed text-muted-foreground"><strong className="text-foreground">Check the result:</strong> {outputCheck(prompt)}</p>
    </article>
  );
}
...
export default function SeoPrompts() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <nav aria-label="Suggested SEO workflow" className="mb-8 border-y border-white/15 py-5">
      ...
      <div className="mb-6">
        ...
        <div className="mt-4 flex flex-wrap gap-2" role="group" aria-label="Filter by SEO task">
          {CATEGORIES.map(category => <button type="button" key={category} aria-pressed={activeCategory === category} onClick={() => chooseCategory(category)} className={`${button} ${activeCategory === category ? 'border-foreground bg-white/10 text-foreground underline decoration-2 underline-offset-4' : 'text-muted-foreground'}`}>{category}</button>)}
        </div>
      </div>
      ...
      {filtered.length === 0 && <div className="mb-10 rounded-lg border border-white/15 p-6">
      ...
      <section aria-labelledby="prompt-review-help" className="mb-8 rounded-lg border border-white/20 p-6 sm:p-8">
      ...
      <section aria-labelledby="prompt-use-help" className="max-w-3xl">
        ...
        <p className="mt-4 text-sm text-muted-foreground">For a longer walkthrough, read <Link href="/blog/chatgpt-prompts-for-seo/" className="text-brand underline underline-offset-4 focus-visible:outline-2 focus-visible:outline-brand">ChatGPT prompts for SEO</Link>.</p>
      </section>
    </div>
  );
}
