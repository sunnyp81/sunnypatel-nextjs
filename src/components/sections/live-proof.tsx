  ...
  return (
    ...
    >
      ...
      <line
        ...
        x2={width - padX}
        y2={height - padBottom}
        className="stroke-hairline-strong dark:stroke-white/10"
        strokeWidth={1}
      />
      <path d={path} fill="none" stroke="var(--brand-ink)" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
      {points.map((p, i) => (
        <g key={`${i}-${p.value}`}>
          <circle cx={p.x} cy={p.y} r={4} fill="var(--brand-ink)" />
          <text x={p.x} y={p.y - 12} textAnchor="middle" className="fill-ink-faint text-[10px] dark:fill-white/50">
            {p.value}
          </text>
        </g>
      ))}
...
export function LiveProof({ proof }: { proof: ProofData }) {
  ...
  return (
    <section className="relative overflow-hidden border-t border-hairline bg-background py-24 md:py-32 dark:border-white/[0.05] dark:bg-[#050507]">
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-6">
        ...
        >
          ...
          <Link
            href="/proof/"
            className="whitespace-nowrap text-sm text-brand-ink transition-colors hover:text-foreground dark:hover:text-white"
          >
            Experiment log →
          </Link>
        </motion.div>
        ...
        <motion.div
          ...
          className="rounded-2xl border border-hairline bg-surface-2 p-6 shadow-[var(--elev)] sm:p-8 dark:border-white/[0.08] dark:bg-black/20 dark:shadow-none dark:backdrop-blur-sm"
        >
          ...
          <p className="mt-2 text-xs text-ink-faint dark:text-white/40">
            Source data refreshed {updated}. Transparent test-site snapshot, not a client
            performance claim.
          </p>

          <div className="mt-6 divide-y divide-hairline border-t border-hairline dark:divide-white/[0.06] dark:border-white/[0.06]">
            {proof.rankings.slice(0, 4).map((ranking) => (
              <div key={ranking.query} className="flex items-center justify-between py-2.5 text-sm">
                <span className="text-muted-foreground dark:text-white/70">{ranking.query}</span>
