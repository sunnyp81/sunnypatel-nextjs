...
const navigation = [["statistics", "Find a statistic"], ["uk-search-market", "UK market and spending"], ["agency-numbers", "UK agency numbers"], ["compare-ctr", "Use CTR benchmarks"], ["ai-and-zero-click", "AI and zero-click search"], ["seo-roi", "Build an SEO business case"], ["methodology", "Method and corrections"], ["reuse", "Cite and download"], ["faq", "Common questions"]];
  ...
  return <div className={styles.guide}><Navbar /><main id="main-content" tabIndex={-1}>
    ...
    <div className={styles.layout}><aside className={styles.contents}><nav aria-label="On this page"><p>On this page</p><ol>{navigation.map(([id,label]) => <li key={id}><a href={`#${id}`}>{label}</a></li>)}</ol></nav></aside>
      <div className={styles.body}>
        <section id="statistics" className={styles.section}><h2>Which SEO statistics can you cite?</h2><p>Compare 12 UK findings, three US comparisons and four portfolio observations. Copy a citation with its original source and limitation, or use the permanent link to reference one entry.</p><StatisticsLibrary /></section>
