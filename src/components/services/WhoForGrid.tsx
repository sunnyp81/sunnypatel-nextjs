      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <ul className="space-y-2.5">
            {yesItems.map((item, i) => (
              <li key={i} className="flex items-start gap-2.5 text-sm text-muted-foreground">
                <span className="mt-0.5 shrink-0 font-bold text-emerald-700 dark:text-emerald-400">✓</span>
                {item}
              </li>
            ))}
          </ul>
        ...
        <div className="rounded-2xl border border-red-500/20 bg-red-500/[0.04] p-5 shadow-[0_0_20px_rgba(239,68,68,0.06)]">
          ...
          <ul className="space-y-2.5">
            {noItems.map((item, i) => (
              <li key={i} className="flex items-start gap-2.5 text-sm text-muted-foreground">
                <span className="mt-0.5 shrink-0 font-bold text-red-700 dark:text-red-400">✗</span>
                {item}
              </li>
            ))}
          </ul>
